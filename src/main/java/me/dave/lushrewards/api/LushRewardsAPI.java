...
public class LushRewardsAPI {
    ...
    public static void registerRewardType(String type, Reward.Constructor constructor) {
        PlatyUtils.getManager(RewardManager.class).ifPresent(manager -> manager.register(type, constructor));
    }

    public static void unregisterRewardType(String type) {
        PlatyUtils.getManager(RewardManager.class).ifPresent(manager -> manager.unregister(type));
    }
}
